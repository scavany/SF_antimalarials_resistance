# packages
# install.packages(c("lhs","sensitivity","pbapply"))
library(lhs)
library(sensitivity)
library(pbapply)
library(deSolve)
...
# 1) Define parameter names and ranges
# params <- list(
#   qq = c(0.1, 0.3),
#   wait_treat = c(1 / 24 / 61, 1 / 24 / 59),
#   R0 = c(2.249, 2.51),
#   gammaa = c(0.749, 0.751),
#   gammap = c(0.749, 0.751),
#   nuDmin = c(365 / 28, 365 / 27),
#   nupmax = c(365 / 2.01, 365 / 1.99)
# )
...
MedQual<-function(t, X, parameters) {
    with(as.list(c(X, parameters)), {
        ...
        beta <- R0*(muo+nu)*seas                                      
        lam <- beta*(IC1+kA*(IA1+T1+T2+T3+Tp)+kU*(IU1+Tr))/N   # force of infection 
        nuD <- nuDmin / (dose / parameters$Ds + 0.01 / 365)                # clearance rate on treatment day 4+
        # nuD <- nuDmin * (1 - dose / parameters$Ds) + nuDmax * (dose / parameters$Ds)
        # print(nuD)
        ...
        fw <- ((t>tf)*rf)*f                                         # follow-up testing rate
        ...
        cpmax[1] <- c.africa[,4]<-1-p.africa[,4]/p.africa[,3]  
...
run_model <- function(pars) {
    ...
    parameters <- list(
        ...
        mui=1/50,                    # birth rate
        muo=1/50,                    # death rate
        R0=rep(pars$R0, A),              # basic reproduction number (set at low level)
        ...
        wait_treat = pars$wait_treat, # 0.5,              # average wait time before treatment after clinical symptoms (days)
        ...
        nupmax=pars$nupmax,                # clearance by partner drug after day 3 of treatment
        ...
        nuDmin=c(pars$nuDmin, pars$nuDmin * 27 / 10),     # min rate of elimination of perfect partner-drug after 3 days of artmeisinin ACT
        # nuDmax=c(365/4,365/4),       # max rate of elimination of perfect drug after 3 days of artmeisinin ACT
        # nuD = c(pars$nuD, pars$nuD * 27 / 10),                # rate of elimination of drug after 3 days of artmeisinin ACT (partner drug)  
        ...
        lam=rep(0, A),               # ?
        gammaa = pars$gammaa,               # degree of artemisinin resistance
        gammap = pars$gammap,                # degree of partner-drug resistance
        Sindex = Sindex,
        T1index = T1index,
        T2index = T2index,
        T3index = T3index,
    ...
    cinc<-sum(CumInc[length(CumInc[,2]),])-(prod(CumInc[length(CumInc[,2]),]))/((maxt-parameters$t_treat)*1000)
    cincres<-as.numeric(CumInc[length(CumInc[,2]),2])
    clinc<-sum(CumIC1[length(CumIC1[,2]),])-(prod(CumIC1[length(CumIC1[,2]),]))/((maxt-parameters$t_treat)*1000)
    clincres<-as.numeric(CumIC1[length(CumIC1[,2]),2])
    cliFail<-sum(CliFail[length(CliFail[,2]),])-(prod(CliFail[length(CliFail[,2]),]))/((maxt-parameters$t_treat)*1000)
    return(c(cinc = cinc, clinc = clinc, cliFail = cliFail))
}
##----- end loop ---------------------------------------------------------------------------------------------------------##    

# 3) Run model for each sample
run_single <- function(pars_vec) {
...
results <- pbapply(param_mat, 1, run_single)
results_df <- as.data.frame(t(results))
combined <- cbind(as.data.frame(param_mat), results_df)

# parameter names (in the order of columns in param_mat)
param_names <- colnames(param_mat)
k <- length(param_names)

# outcome names
outcome_names <- colnames(results_df)

# ensure combined is numeric for params and outcomes
...
# loop over outcomes and parameters
for (oj in seq_along(outcome_names)) {
  y <- combined[[ outcome_names[oj] ]]
  for (i in seq_len(k)) {
    x <- combined[[ param_names[i] ]]
    # set Z = all other parameters (matrix) to partial out
    other_idx <- setdiff(seq_len(k), i)
    Z <- as.matrix(combined[, other_idx, drop = FALSE])
    # pcor.test expects numeric vectors and matrix Z; method = "spearman" gives PRCC
    test <- tryCatch(
      pcor.test(x, y, Z, method = "spearman"),
      error = function(e) NULL
    )
    if (!is.null(test)) {
      prcc_est[i, oj] <- test$estimate    # partial correlation
...
# Convert to data.frames for easier printing/plotting
prcc_est_df <- as.data.frame(prcc_est)
prcc_p_df   <- as.data.frame(prcc_p)

# Example: print PRCC estimates for primary outcome (first outcome)
print(prcc_est_df[,3])
print(prcc_p_df[,3])

# model function
### size of array: A=2 represent the two array (two layers: sensitive, resistance) 
save(prcc_est_df, prcc_p_df, file = "./prcc_results.RData")
